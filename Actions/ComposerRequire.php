...
class ComposerRequire extends AbstractComposerAction implements iModifyData
{
    ...
    protected function performComposerAction(ComposerAPI $composer)
    {
        if (! $this->getInputDataSheet()->getMetaObject()->isExactly('axenox.PackageManager.PACKAGE_INSTALLED')) {
            throw new ActionInputInvalidObjectError($this, 'Wrong input object for action "' . $this->getAliasWithNamespace() . '" - "' . $this->getInputDataSheet()->getMetaObject()->getAliasWithNamespace() . '"! This action requires input data based based on "axenox.PackageManager.PACKAGE_INSTALLED"!', '6T5E8Q6');
        }
